const db = require('../config/db')
const path = require('path')
const fs = require('fs')

const deleteFile = (imgPath) => {
  if (!imgPath) return
  const full = path.join(__dirname, '../', imgPath.replace(/^\//, ''))
  if (fs.existsSync(full)) fs.unlinkSync(full)
}

// GET all testimonials
exports.getAllTestimonials = (req, res) => {
  db.query('SELECT * FROM testimonials ORDER BY created_at DESC', (err, results) => {
    if (err) return res.status(500).json({ success: false, message: err.message })
    res.json({ success: true, data: results })
  })
}

// POST create testimonial
exports.createTestimonial = (req, res) => {
  const { name, position, message, rating, status } = req.body
  if (!name || !message) return res.status(400).json({ success: false, message: 'Name and message required' })
  const image = req.file ? `/uploads/testimonials/${req.file.filename}` : null

  db.query(
    'INSERT INTO testimonials (name, position, message, rating, image, status) VALUES (?,?,?,?,?,?)',
    [name, position || null, message, rating || 5, image, status || 'Active'],
    (err, result) => {
      if (err) return res.status(500).json({ success: false, message: err.message })
      res.status(201).json({ success: true, message: 'Testimonial created', id: result.insertId })
    }
  )
}

// PUT update testimonial
exports.updateTestimonial = (req, res) => {
  const { id } = req.params
  const { name, position, message, rating, status } = req.body

  db.query('SELECT image FROM testimonials WHERE id = ?', [id], (err, rows) => {
    if (err) return res.status(500).json({ success: false, message: err.message })
    if (!rows.length) return res.status(404).json({ success: false, message: 'Not found' })

    let image = rows[0].image
    if (req.file) {
      deleteFile(image)
      image = `/uploads/testimonials/${req.file.filename}`
    }

    db.query(
      'UPDATE testimonials SET name=?, position=?, message=?, rating=?, image=?, status=? WHERE id=?',
      [name, position || null, message, rating || 5, image, status || 'Active', id],
      (err2) => {
        if (err2) return res.status(500).json({ success: false, message: err2.message })
        res.json({ success: true, message: 'Testimonial updated' })
      }
    )
  })
}

// DELETE testimonial
exports.deleteTestimonial = (req, res) => {
  const { id } = req.params
  db.query('SELECT image FROM testimonials WHERE id = ?', [id], (err, rows) => {
    if (err) return res.status(500).json({ success: false, message: err.message })
    if (!rows.length) return res.status(404).json({ success: false, message: 'Not found' })

    db.query('DELETE FROM testimonials WHERE id = ?', [id], (err2) => {
      if (err2) return res.status(500).json({ success: false, message: err2.message })
      deleteFile(rows[0].image)
      res.json({ success: true, message: 'Deleted' })
    })
  })
}
