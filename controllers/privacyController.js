...
const ensureTable = () => {
  ...
  `, (err) => { if (err) console.log('privacy_page table error:', err.message) })
}
ensureTable()
...
exports.getPrivacyPage = (req, res) => {
  db.query('SELECT * FROM privacy_page WHERE id = 1', (err, rows) => {
    if (err) return res.status(500).json({ success: false, message: err.message })
    res.json({ success: true, data: rows.length ? parseRow(rows[0]) : defaults })
  })
}

// GET (public) — same data, consumed by the /datenschutz page
exports.getPublicPrivacyPage = (req, res) => {
  db.query('SELECT * FROM privacy_page WHERE id = 1', (err, rows) => {
    if (err) return res.status(500).json({ success: false, message: err.message })
    res.json({ success: true, data: rows.length ? parseRow(rows[0]) : defaults })
  })
...
exports.updatePrivacyPage = (req, res) => {
  ...
  db.query(
    `INSERT INTO privacy_page (id, title_de, title_en, subtitle_de, subtitle_en, sections)
     VALUES (1, ?, ?, ?, ?, ?)
    ...
    (err) => {
      if (err) return res.status(500).json({ success: false, message: err.message })
      res.json({ success: true, message: 'Privacy page updated successfully' })
    }
  )
}
